 ...
 * - Firestore holds `Timestamp` — every save normalises through
 *   `toExpiryTimestamps` [BUG-018] — and the SDK reads it back as a `Timestamp`
 ...
 * instant to store.
 *
 * Used by the permissions editor when an admin picks an expiry date [BUG-018].
 *
