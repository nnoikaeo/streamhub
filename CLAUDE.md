...
| `npm run docs:links` | Read-only Markdown link check across every tracked `.md` (skips `.claude/skills/`, which is vendored). Verifies the file exists **and**, for `.md` targets, that the `#anchor` still matches a heading — GitHub's slug rules, so an emoji heading anchors as `#-tag-permissions`. A `#L120` on a `.ts`/`.vue` target is a line ref, not an anchor, so only the file is checked. Enforced by [.github/workflows/docs.yml](.github/workflows/docs.yml) on any PR that touches a `.md`. **Baseline is 0** (PR fixing the original 40) |
