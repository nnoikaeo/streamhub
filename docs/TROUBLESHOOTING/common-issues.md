...
## Issue: แดชบอร์ด Looker ขึ้น "เข้าถึงรายงานไม่ได้" บน Safari ทุกเครื่อง (BUG-032, 2026-08-23)
...
**บน Chrome เดสก์ท็อปปกติดี** — นี่คือสิ่งที่ทำให้ปัญหานี้ซ่อนอยู่นาน

**อ่านอาการให้ถูกก่อน — Looker ขึ้นได้ 3 แบบ คนละสาเหตุ**

| ข้อความในกรอบ iframe | แปลว่า | แก้ที่ |
|---|---|---|
| "เว็บไซต์นี้ใช้คุกกี้จาก Google — อนุญาตคุกกี้ของบุคคลที่สาม" | Safari บล็อกคุกกี้ · รายงานยังแชร์แบบจำกัดสิทธิ์ | เปลี่ยนการแชร์เป็น "ใครมีลิงก์ก็ดูได้" |
| **"เจ้าของรายงานปิดใช้การดูในเว็บไซต์อื่น"** | แชร์ลิงก์แล้วแต่ **embedding ปิดอยู่** — ขึ้นทุกเบราว์เซอร์ ไม่เกี่ยวกับคุกกี้ | File > Embed report → Enable embedding |
| กรอบขาวเปล่า ไม่มีข้อความเลย | ไม่ใช่สองอันบน — บัญชีที่เปิดไม่มีสิทธิ์ในรายงานนั้นเลย หรือรายงานว่าง | ดูที่ตัวรายงานใน Looker |

> ช่อง Looker Studio URL ในกล่องแก้ไขแดชบอร์ดมี preview ในตัว ⇒ เห็นข้อความพวกนี้ได้ตั้งแต่ตอนวาง URL ก่อนกดบันทึก

**สาเหตุ** — Looker ยืนยันสิทธิ์ผู้ดูด้วยคุกกี้ของ Google ซึ่งใน iframe ของเราเป็น **คุกกี้บุคคลที่สาม** และ Safari บล็อกไว้โดยปริยาย ("ป้องกันการติดตามข้ามไซต์") · **ไม่ใช่ 403 ของ embed token** ถ้า token พังจะเป็นหน้า 403 ของเราเอง

**สิ่งที่ตัดทิ้งไปแล้ว (อย่าเสียเวลาซ้ำ)**

| ลองแล้ว | ผล |
|---|---|
| ล็อกอิน `lookerstudio.google.com` แบบ first-party บนเครื่องเดียวกัน | **ไม่ช่วย** — ไม่ใช่เรื่อง first-party interaction heuristic |
| ปิด "ป้องกันการติดตามข้ามไซต์" | ขึ้นทันที — ยืนยันว่าเป็นคุกกี้บุคคลที่สามล้วน ๆ (ใช้ยืนยันสาเหตุเท่านั้น ไม่ใช่ทางแก้ที่จะบอกผู้ใช้) |
| เติม `allow-storage-access-by-user-activation` ใน `sandbox` | อยู่ในโค้ดแล้วแต่ **วัดบน prod ไม่เปลี่ยนอะไร** — Looker ไม่เรียก `requestStorageAccess()` เอง |

**ทางแก้ที่ใช้ได้จริง** — ที่ตัวรายงานใน Looker Studio ต้องเปิด **สองสวิตช์**:

1. แชร์เป็น "ใครมีลิงก์ก็ดูได้"
2. **File > Embed report > Enable embedding**

ยืนยันบน iPad 9 แล้วว่ารายงานที่เปิดครบสองอย่างนี้แสดงผลได้ทั้งที่ "ป้องกันการติดตามข้ามไซต์" เปิดอยู่

> ⚠️ **รายงานที่แชร์ลิงก์แต่ไม่เปิด embedding = กรอบขาวทุกเบราว์เซอร์** รวม Chrome บนแมคที่ไม่บล็อกคุกกี้อะไรเลย · หน้าตาเหมือนอาการคุกกี้มาก เคยทำให้เกือบสรุปผิดว่าการแชร์ลิงก์ใช้ไม่ได้ · เช็คแยกด้วยการเปิด embed URL ตรง ๆ ในแท็บใหม่

> ⚠️ **ราคาที่ต้องจ่าย** — ใครถือลิงก์ Looker เปิดรายงานได้โดยไม่ผ่านสิทธิ์ของ StreamHub · ตัว URL ยังถูกปิดผนึกอยู่ใน embed token ไม่หลุดถึง client แต่ถ้าหลุดทางอื่นก็เปิดได้จริง ⇒ **ตัดสินใจเป็นรายรายงานตามความอ่อนไหวของข้อมูล**

**ผู้ใช้เจอแล้วต้องทำยังไง** — หน้าแดชบอร์ดมีแถบคำแนะนำขึ้นเองบนเบราว์เซอร์ WebKit ([browser.ts](../../app/utils/browser.ts)) บอกวิธีผ่อนคุกกี้ · แถบขึ้นกับรายงานที่ใช้งานได้ด้วย เพราะฝั่งเราแยกไม่ออกว่ารายงานไหนพัง (cross-origin) · กด ✕ ปิดถาวรได้

---

## Issue: Google Sheets ที่ฝังขึ้นหน้า "อนุญาตให้ Google ชีต เข้าถึงคุกกี้ที่จำเป็น" (2026-09-06)

**อาการ** — แดชบอร์ดชนิด Google Sheets เปิดบน Safari / iPhone / iPad แล้วในกรอบขึ้นหน้าของ Google เองว่า *"อนุญาตให้ Google ชีต เข้าถึงคุกกี้ที่จำเป็น"* พร้อมปุ่ม **"อนุญาตคุกกี้"** · **กดปุ่มนั้นแล้วตัน** ไม่ไปไหนต่อ · บน Chrome เดสก์ท็อปของคนที่เป็นเจ้าของชีต **เปิดได้ปกติ** จึงหลอกตาว่าไม่มีปัญหา

**สาเหตุ** — ชีตใบนั้น**ยังไม่ได้แชร์แบบ "ทุกคนที่มีลิงก์ (ผู้ดู)"** ⇒ Google ต้องยืนยันตัวผู้ดูด้วยคุกกี้ ซึ่งใน iframe เป็นคุกกี้บุคคลที่สามที่ Safari บล็อก · กลไกเดียวกับ [BUG-032 ของ Looker](#issue-แดชบอร์ด-looker-ขึ้น-เข้าถึงรายงานไม่ได้-บน-safari-ทุกเครื่อง-bug-032-2026-08-23) แต่คนละหน้าจอ

**ทางแก้ทางเดียว** — เปิดชีต → **Share** → General access → **ทุกคนที่มีลิงก์ (ผู้ดู)** · ต่างจาก Looker ตรงที่**ไม่มีสวิตช์ Enable embedding** ให้กด (Google ไม่ส่ง `X-Frame-Options` บน Sheets เลย) ⇒ มีสวิตช์เดียวต้องกด

**ระบบตรวจให้แล้ว ไม่ต้องเดา** — ช่อง Google Sheets URL ในฟอร์มยิง `POST /api/sheet/check-sharing` อัตโนมัติ (debounce 800ms) · เซิร์ฟเวอร์ขอ `export?format=csv` ของชีตนั้น **โดยไม่แนบ credential ใด ๆ** แล้วดูสถานะ:

| สถานะ | แปลว่า | ฟอร์มทำอะไร |
|---|---|---|
| 200 | แชร์ลิงก์แล้ว ฝังได้ | บันทึกได้ พร้อมคำเตือนเรื่องดาวน์โหลดทั้งไฟล์ |
| 401 | ยังไม่ได้แชร์ลิงก์ | **บล็อกการบันทึก** (BUG-034) |
| ยิงไม่ถึง Google | ตอบไม่ได้ | ไม่บล็อก แต่บอกว่าตรวจไม่ได้ |
...
## Issue: กดบันทึกแล้วปุ่มค้าง "กำลังบันทึก..." ไม่มีข้อความอะไรเลย

**อาการ:** ฟอร์ม admin กดบันทึกแล้วปุ่มจางค้าง ไม่มี toast ทั้งเขียวทั้งแดง รอเป็นนาทีก็ไม่เปลี่ยน

**สาเหตุ:** เน็ตหลุด · หน้า admin เขียน Firestore ตรงผ่าน SDK ซึ่ง `setDoc` จะ resolve ต่อเมื่อ server ตอบรับ และ **ไม่มี timeout** ⇒ ระหว่างออฟไลน์งานถูก queue ไว้เงียบ ๆ (BUG-026)

**ตั้งแต่ 2026-08-20 มีแถบเหลืองบอกแล้ว** — ถ้าเห็น "⚠️ ออฟไลน์ — การบันทึกจะค้างจนกว่าจะกลับมาออนไลน์" บนสุดของหน้า แปลว่าเป็นเคสนี้ · ถ้าปุ่มค้างโดย**ไม่มี**แถบนั้น แสดงว่าเบราว์เซอร์ยังคิดว่าออนไลน์อยู่ (เช่น Wi-Fi ติดแต่เน็ตไม่ออก) ให้ดู tab Network ว่า request ค้างที่ `firestore.googleapis.com` หรือไม่

**งานหายไหม:** ไม่หาย — พอเน็ตกลับมา SDK ส่งคิวที่ค้างให้เอง แล้ว toast เขียวจะเด้งตามมา modal ปิดเอง

**ทำยังไง:** เช็คเน็ตแล้วรอ อย่ากด F5 ระหว่างนั้น (แอปเป็น SPA ไม่มี service worker รีโหลดตอนออฟไลน์ = จอว่าง) · ยืนยันด้วยการทดสอบจริงแล้ว ดู TC 6.3.2 ใน [manual-test-plan.md](../OPERATIONS/manual-test-plan.md)

> **ทำไมไม่ใส่ timeout ให้ปุ่ม:** ตัด promise ทิ้งหลัง N วินาที = โกหกว่าล้มเหลว ทั้งที่ SDK ยังส่งงานนั้นสำเร็จเมื่อเน็ตกลับมา ผู้ใช้จะกดซ้ำแล้วได้ข้อมูลซ้ำแทน

---

## Issue: ลด role moderator เป็น user แล้วโฟลเดอร์ที่เขาดูแลหายหมด

**อาการ:** เปลี่ยน role เป็น `user` แล้วเปลี่ยนกลับเป็น `moderator` ปรากฏว่าโฟลเดอร์ที่เคยดูแลไม่กลับมา

**สาเหตุ:** ตอนลด role ระบบถอด uid ออกจาก `assignedModerators` ของทุกโฟลเดอร์ ([folderAssignment.ts](../../app/utils/folderAssignment.ts)) — เป็นพฤติกรรมตั้งใจ แต่ไม่มีที่เก็บประวัติ จึงคืนให้อัตโนมัติไม่ได้ (BUG-027)

**ตั้งแต่ 2026-08-20 ฟอร์มถามก่อนแล้ว** — กดบันทึกแล้วจะขึ้น "ดูแลอยู่ N โฟลเดอร์ — เปลี่ยนบทบาทแล้วจะถูกถอดออกจากทุกโฟลเดอร์ และเลื่อนกลับเป็น moderator ภายหลังจะไม่ได้คืนอัตโนมัติ" · กดยกเลิกแล้วไม่มีอะไรถูกเขียน

**ทำยังไง:** ผูกโฟลเดอร์คืนที่ Explorer (ปุ่มมอบหมายผู้ดูแล) · ก่อนลด role ของ moderator ให้จดไว้ก่อนว่าเขาดูแลอะไรอยู่ — ดูได้ที่หน้า `/profile` ของเขา หรือกรองจาก `assignedModerators`
