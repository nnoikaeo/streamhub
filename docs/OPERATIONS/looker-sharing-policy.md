# นโยบายการแชร์รายงาน Looker

> สร้าง 2026-08-24 เป็นเช็กลิสต์ · **เปลี่ยนเป็นนโยบาย 2026-08-25** เพราะ 30 รายงานที่ใช้อยู่แก้ไม่ได้ (ดูด้านล่าง) · ที่มา: [BUG-032](manual-test-plan.md) · พื้นหลัง: [common-issues.md](../TROUBLESHOOTING/common-issues.md)

## กฎสำหรับแดชบอร์ดใหม่ทุกตัว

**รายงาน Looker ที่จะเอามาฝังใน StreamHub ต้องตั้ง "ใครมีลิงก์ก็ดูได้" + เปิด Enable embedding** ตั้งแต่ตอนสร้าง ไม่ใช่ตามแก้ทีหลัง · ตกลงไว้ 2026-08-25

> กฎนี้ขึ้นเตือนในแอปด้วย — พอวาง URL ที่ถูกต้องในกล่องสร้าง/แก้ไขแดชบอร์ด [LookerUrlInput.vue](../../app/components/features/LookerUrlInput.vue) จะโชว์สองข้อนี้ใต้ช่อง พร้อมบอกอาการที่จะเจอถ้าข้ามข้อไหนไป · กด Preview ตรงนั้นดูผลจริงได้ก่อนบันทึก

## ⛔ 30 รายงานเดิมแก้ไม่ได้ — และจะเป็นแบบนี้ต่อไป

เจ้าของโปรเจกต์ StreamHub **ไม่ได้เป็นผู้สร้างหรือผู้ดูแล** รายงาน 30 ตัวที่ระบบใช้อยู่ ⇒ เปลี่ยนการแชร์เองไม่ได้

**สิ่งที่ตามมา ไม่ใช่ปัญหาชั่วคราว:**

- ผู้ใช้ **Safari ทุกเครื่อง** (macOS, iPhone, iPad รวมถึง Chrome/Firefox บน iOS ซึ่งเป็น WebKit) **เปิดแดชบอร์ดเหล่านี้ไม่ได้เลย** จนกว่าจะปิด "ป้องกันการติดตามข้ามไซต์" ของตัวเอง หรือย้ายไปใช้ Chrome เดสก์ท็อป
- **แถบคำแนะนำในหน้าแดชบอร์ดคือทางออกถาวร ไม่ใช่ของขัดตาทัพ** ([browser.ts](../../app/utils/browser.ts)) — อย่าถอดออกจนกว่ารายงานเดิมจะถูกเปลี่ยนหรือเลิกใช้
- ถ้าอยากปิดช่องนี้จริง ต้องไปคุยกับ**เจ้าของรายงาน**ให้เปลี่ยนการแชร์ให้ — เป็นเรื่องข้ามทีม ไม่ใช่งานที่แก้ได้จากฝั่งโค้ด

รายการ 30 ตัวเก็บไว้ท้ายเอกสารเผื่อวันหนึ่งคุยกับเจ้าของรายงานสำเร็จ จะได้ไล่ได้ทันทีว่าตัวไหนบ้าง

## Google Sheets ใช้กฎคนละชุด

ตัดสินใจ 2026-09-06 ว่าจะฝัง Google Sheets ด้วย iframe เหมือนกัน แต่**ยังไม่ได้สร้าง** · กฎของชีตอยู่ใน [google-sheets-embed-plan.md](google-sheets-embed-plan.md) ที่มาของกฎอยู่ใน [google-sheets-spike-plan.md](google-sheets-spike-plan.md)

ต่างจาก Looker สองข้อ: ชีต**ไม่มี**สวิตช์ Enable embedding ให้กด (Google ไม่บล็อกการฝังเลย) แต่ถ้าลิงก์หลุด คนที่ถือ URL **โหลดได้ทั้งไฟล์** เป็น csv/xlsx/pdf ไม่ใช่แค่ดูรายงาน ⇒ ข้อมูลอ่อนไหวต้องให้เจ้าของข้อมูลตัดสินก่อน ไม่ใช่ตัดสินตอนเพิ่มแดชบอร์ด
...
(ใช้กับรายงานใหม่ที่เราคุมได้เอง)
...
## สิ่งที่แลกไป — รู้ไว้ก่อนตั้งค่ารายงานใหม่
...
## ยืนยันหลังตั้งค่ารายงานใหม่

1. iPad/iPhone Safari — **เปิด** "ป้องกันการติดตามข้ามไซต์" ไว้ (ค่าปริยาย) · ปิดสวิตช์ไว้แล้วทดสอบ = ไม่พิสูจน์อะไร
2. เปิดแดชบอร์ดที่เพิ่งเพิ่ม ต้องเห็นรายงานจริงทันที ไม่ใช่หน้า "เข้าถึงรายงานไม่ได้" และไม่ใช่กรอบขาว
3. เร็วกว่านั้น: ดู preview ในกล่องแก้ไขแดชบอร์ดตั้งแต่ตอนวาง URL ยังไม่ต้องกดบันทึก
