# นโยบายการแชร์รายงานที่ฝัง (Looker และ Google Sheets)

> สร้าง 2026-08-24 เป็นเช็กลิสต์ · **เปลี่ยนเป็นนโยบาย 2026-08-25** เพราะ 30 รายงานที่ใช้อยู่แก้ไม่ได้ (ดูด้านล่าง) · **เพิ่มกฎของ Google Sheets 2026-09-06** ตอนฝัง Sheets ได้จริง · ชื่อไฟล์ยังเป็น `looker-sharing-policy.md` เพราะมีลิงก์เข้ามาหลายที่
>
> ที่มา: [BUG-032](manual-test-plan.md) · พื้นหลัง: [common-issues.md](../TROUBLESHOOTING/common-issues.md)

## กฎสำหรับแดชบอร์ด Looker ใหม่ทุกตัว
...
**ชีตต้องแชร์เป็น "ทุกคนที่มีลิงก์ (ผู้ดู)" เท่านั้น** ไม่มีทางเลือกอื่น — ชีตที่ผูกกับบัญชีเปิดใน iframe บน Safari/iOS ไม่ได้เลย และปุ่ม "อนุญาตคุกกี้" ที่ Google ขึ้นให้ในกรอบ **กดแล้วตัน**

**เจ้าของข้อมูลต้องอนุมัติก่อน ไม่ใช่ตัดสินตอนเพิ่มแดชบอร์ด** — ดู "สิ่งที่แลกไป" ด้านล่าง

> กฎนี้ตรวจให้ในแอป — [SheetUrlInput.vue](../../app/components/features/SheetUrlInput.vue) มีปุ่ม **ตรวจการแชร์** ที่ยิงไปที่ `POST /api/sheet/check-sharing` · เซิร์ฟเวอร์ขอ `export?format=csv` ของชีตนั้น **โดยไม่แนบ credential ใด ๆ** แล้วดูสถานะ: 200 = แชร์ลิงก์แล้ว · 401 = ยัง
>
> ต้องตรวจ เพราะอาการนี้คนสร้างแดชบอร์ดมองไม่เห็นเอง — ชีตที่ยังไม่แชร์ยัง**แสดงผลปกติให้เจ้าของชีตเองใน Chrome** (เซสชัน Google เดียวกัน) แล้วไปพังกับผู้ใช้จริงบน Safari

### ต่างจาก Looker สามข้อ

| | Looker | Sheets |
|---|---|---|
| สวิตช์ Enable embedding | **ต้องกด** ไม่กดแล้วกรอบว่างทุกเบราว์เซอร์ | **ไม่มี** — Google ไม่ส่ง `X-Frame-Options` บน Sheets เลย |
| ลิงก์หลุดแล้วได้อะไร | รายงานที่ render แล้ว | **ทั้งไฟล์** — `export?format=csv\|xlsx\|pdf` และ `gviz` ตอบ 200 ให้คนที่ไม่ล็อกอิน |
| ตรวจอัตโนมัติได้ไหม | ไม่ได้ ต้องดู preview เอา | ได้ ปุ่มตรวจการแชร์ในฟอร์ม |

### สิ่งที่แลกไป — หนักกว่าของ Looker

ลิงก์ที่แชร์แล้ว **= ไฟล์ทั้งใบ** ไม่ใช่แค่ภาพรายงาน · วัดจริงแล้ว: ก่อนแชร์ 401 ทุก endpoint หลังแชร์ 200 ทุก endpoint (`preview` 46 KB · `csv` 544 B · `gviz` 635 B · `xlsx` 5.6 KB · `pdf` 95 KB) ⇒ ใครถือ URL ดาวน์โหลดเป็นไฟล์ได้เลย ไม่ต้องเปิดเบราว์เซอร์ ไม่ต้องล็อกอิน

- **ยังคุมได้:** URL จริงถูกผนึกใน [embed token](../../server/utils/embedToken.ts) เหมือน Looker และถูกตัดออกจาก API listing ทุกเส้น ([embedUrl.ts](../../shared/utils/embedUrl.ts))
- **เสียไปจริง:** กันได้แค่ทางแอป · ใครได้ URL มาทางอื่นก็โหลดไฟล์ได้
- ⚠️ **ชีตที่มีข้อมูลระดับเดียวกับ `Master List`** (รายชื่อร้าน เลขสัญญา รายราย) **ไม่ควรเข้าทางนี้เลย**

### หมายเหตุเรื่องโหมด `interactive`

โหมดเริ่มต้นฝัง `/edit?rm=minimal` ซึ่งแปลว่า **คนที่มีสิทธิ์แก้ไขบนชีตนั้นแก้ได้จริงผ่านกรอบ** · ชีตที่แชร์ลิงก์เป็น "ผู้ดู" แก้ไม่ได้ แต่เจ้าของที่เปิดแดชบอร์ดเองจะแก้ได้ — ตั้งใจให้เป็นแบบนั้น ถ้าไม่ต้องการให้เลือกโหมด `view` (`/preview`) ซึ่งเป็นตารางแบนคลิกไม่ได้

### แถบเตือน WebKit ไม่ขึ้นกับแดชบอร์ดชนิด sheet

ชีตที่ผ่านการตรวจการแชร์แล้วเปิดบน Safari ได้ตามปกติ ⇒ ขึ้นแถบทั้งที่ใช้งานได้จะสอนผู้ใช้ผิด · แถบยังขึ้นกับ Looker เหมือนเดิม เพราะ 30 รายงานเดิมแก้ไม่ได้
