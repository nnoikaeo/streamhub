...
- [ ] **`no-explicit-any` — 17 left** — 15 `app/`, 1 `server/`, 1 `scripts/`; `tests/` is clean. The permission path is done; what remains is unrelated leftovers — `useAdminResource` (6), `FormModal` (2), and single sites in `FormField`, `useAuth`, `useDashboardPage`, `errorMessages`, `dashboard` store (2), `seed-firestore`, `dashboards.post`, `bulkInvite`. `app/stores/dashboard.ts` needs a data-model decision first: `(d as any).company` covers a real gap, since neither `Dashboard` nor `Folder` declares a top-level `company`
