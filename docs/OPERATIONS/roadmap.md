...
- [ ] **`no-explicit-any` — 57 left** — 37 `app/`, 19 `server/`, 1 `scripts/`; `tests/` is clean. What remains is almost entirely **the permission path**: `companyAccess.ts` (17) still exposes `filterAccessibleDashboards(dashboards: any[], user: any, folders: any[])` and `checkDashboardAccess(dashboard: any, user: any, …)`, with `useDashboardService` (14), `useFirestoreService` (5) and `useJSONMockService` (4) behind it. Highest regression risk of anything left; do it on its own
