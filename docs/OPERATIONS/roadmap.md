...
- [ ] **`no-explicit-any` — 3 left, each a decision not a rename** — `app/stores/dashboard.ts:73,81` (`(d as any).company`: neither `Dashboard` nor `Folder` declares a top-level `company`, so this is a data-model call) and `useAdminResource`'s `[extensionMethod: string]: any` (needs the composable generic over its extensions map; all eight call sites pass `<T>` explicitly, and TypeScript will not mix explicit and inferred type arguments)
