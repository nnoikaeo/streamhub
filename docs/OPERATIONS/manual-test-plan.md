...
|------------|-------|------------|--------|
| Mobile (drawer) | 375px | ปุ่ม ☰ + drawer + overlay + ย่อโลโก้ + ซ่อนชื่อผู้ใช้ | ✅ (ยืนยันตอนแก้ BUG-025) · ⚠️ ครอบเฉพาะ `AppLayout` — **หน้าแดชบอร์ดมีแถบหัวของตัวเองที่ไม่ได้ถูกตรวจรอบนั้น และพังจริงที่ 375px = BUG-030** แก้แล้ว |
...
เคสไหนพัง จดเป็น BUG-031 ขึ้นไปในตาราง §8 ก่อน แล้วค่อยตัดสินใจว่าจะแก้รอบไหน (029 = ความสูง `<select>` บน WebKit, 030 = ปุ่มซูมถูกตัดที่ 375px)
