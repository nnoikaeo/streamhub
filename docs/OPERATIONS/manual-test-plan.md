...
>
> **id ที่เคสเก่าอ้างถึงจึงเปิดไม่เจอแล้ว** เช่น `dash_1785082599181` (Dashboard in E), `folder_1785082588448` (TEST-E), `folder_finance` · แถวเหล่านั้น**เก็บไว้ตามเดิม**เพราะเป็นบันทึกว่าตอนนั้นทดสอบกับอะไร ไม่ใช่คำสั่งให้ไปเปิดตอนนี้
>
> **จะทดสอบซ้ำต้องสร้างของทิ้งขว้างขึ้นมาใหม่เอง** — สคริปต์ QA สองตัวไม่มีค่าเริ่มต้นแล้ว ต้องระบุเป้าหมายเสมอ (`--dashboard` / `--folder`) เพื่อไม่ให้เผลอไปเขียนทับของจริงที่ตอนนี้เป็นของจริงทั้งหมด
>
> **moderator ไม่มีโฟลเดอร์ที่ดูแลเหลือแล้ว** (`assignedModerators` ว่างทั้ง 12 โฟลเดอร์) ⇒ เคสฝั่ง moderator กดซ้ำไม่ได้จนกว่าจะมอบโฟลเดอร์ให้ใหม่ · **รับสภาพนี้ไว้ก่อน ตั้งใจ ไม่ใช่ของตกหล่น** (ตัดสินใจ 2026-08-25)
...
| BUG-033 | **ปุ่มเลือกชนิดแดชบอร์ด (Looker/Sheets) และปุ่มเลือกโหมดขึ้นสีเดียวกันทุกอัน แยกไม่ออกว่าเลือกอะไรอยู่** — [main.css](../../assets/css/main.css) บังคับ `background-color` + `color` ให้ทุก `<button>` ที่ไม่มีคลาสอยู่ในลิสต์ `:not()` ของกฎนั้น และ specificity ~(0,40,1) ชนะ scoped style ของ component เสมอ · segmented picker ใช้ **สองพร็อพเพอร์ตี้นั้นพอดี** ในการบอกว่าเลือกอันไหน ⇒ ตัวควบคุมไม่แสดง selection เลย ไม่ใช่แค่ "สีเพี้ยน" | P5 ทดสอบมือ (ฟอร์มสร้างแดชบอร์ด) | Medium | 🔧 Fixed (เพิ่ม `.type-button` / `.mode-button` เข้าลิสต์ยกเว้นทั้ง 3 selector ตามวิธีที่ไฟล์เขียนกำกับไว้เอง) — **ยืนยันด้วยตาบน Safari 2026-09-06** ✅ · บทเรียนเดียวกับ BUG-025/BUG-030: กฎ global ตัวนี้กินปุ่มใหม่ทุกตัวที่ไม่ได้ไปขึ้นทะเบียน และ lint/typecheck/เทสต์มองไม่เห็น |
| BUG-034 | **ระบบตรวจได้ว่าชีตยังไม่แชร์ลิงก์ (HTTP 401) แต่ฟอร์มยังบันทึกได้** ⇒ ได้แดชบอร์ดที่เปิดบน Safari/iOS ไม่ได้จริง ซึ่งเป็นเหตุการณ์เดียวกับที่ `POST /api/sheet/check-sharing` ถูกสร้างมากัน · การตรวจถูกทำเป็น**คำเตือน** ไม่ใช่ guard และผูกกับปุ่มที่ต้องกดเอง ⇒ คนไม่กด = ไม่มีอะไรกัน | P5 ทดสอบมือ (สร้างแดชบอร์ดชนิด sheet) | High | 🔧 Fixed ([sheetSharingGuard.ts](../../app/utils/sheetSharingGuard.ts) + 6 เทสต์ — บล็อกการบันทึกเมื่อผลเป็น `not-shared` และระหว่าง `checking` (กันกดบันทึกเร็วกว่าคำตอบ) แต่ **ไม่บล็อกเมื่อ probe ยิงไม่ถึง Google** เพราะปฏิเสธการบันทึกด้วยความล้มเหลวของฝั่งเราเองแย่กว่า · ตรวจอัตโนมัติแบบ debounce 800ms เมื่อ URL แปลงเป็น sheet id ได้ ⇒ เปิดแดชบอร์ดเดิมมาแก้จะตรวจซ้ำให้ ชีตที่ถูกถอนการแชร์ทีหลังจึงโผล่) — **ยืนยันบน Safari 2026-09-06** ✅ ทั้งเส้นบล็อก เส้นบันทึกได้ และข้อความบล็อกหายเองหลังแชร์แล้วตรวจใหม่ |
| BUG-035 | **ตัวกรองบริษัทในหน้า Discover คืนผลว่างเสมอทุกบริษัท** — `access.company` เป็น **array ของรหัสบริษัท** แต่โค้ดอ่านด้วย `in` ซึ่งเช็ค**ดัชนี** ไม่ใช่ค่า ⇒ `'STTH' in ['STTH']` เป็น `false` · เป็นบั๊กชนิดเดียวกับที่แก้ฝั่งเซิร์ฟเวอร์ไปแล้วใน PR #359 แต่ฝั่ง client ตกหล่น และเห็นเฉพาะ admin จึงไม่มีใครรายงาน · [stores/dashboard.ts](../../app/stores/dashboard.ts) มีคอมเมนต์ระบุกฎนี้ไว้แล้วแต่ยังพลาด | ตัวกรองบริษัท (admin) หน้า `/dashboard/discover` | Medium | 🔧 Fixed ([companyFilter.ts](../../app/utils/companyFilter.ts) + 6 เทสต์ — แยกออกมาเป็น util เพราะหน้านั้นไม่มี harness ทดสอบ component และบั๊กนี้โผล่มาแล้ว **สองครั้ง** · รองรับ `ALL` ซึ่งเป็น wildcard จริงที่ [effectiveAccess.ts](../../app/utils/effectiveAccess.ts) ขยายเป็นทุกบริษัท — แก้แค่ `in` → `includes` จะยังซ่อนแดชบอร์ดที่ให้สิทธิ์ `ALL` = ตอบผิดแบบเดิม แค่ผิดน้อยลง) |
