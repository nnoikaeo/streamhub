...
**เกณฑ์ตัดสินว่าคลาสไหนควรอยู่ในลิสต์ — ไม่ใช่ "ดูแล้วพังไหม"** แต่คือ **"คอมโพเนนต์ประกาศ property ที่กฎนี้ตั้งไว้หรือเปล่า"** (`background-color` `color` `border` `border-radius` `padding` `font-weight` · `:hover` → `background-color` `box-shadow` · `:active` → `transform`) · ถ้าประกาศ แปลว่ามีบรรทัดหนึ่งตายอยู่แน่ ๆ และควรให้คอมโพเนนต์ชนะ · ตรวจแบบนี้ได้จากโค้ดล้วน ไม่ต้องเปิดดูทีละหน้า

> การเขียน `background` (shorthand) ในคอมโพเนนต์ไม่ได้ช่วยหนี `background-color` (longhand) ของกฎกลาง — คนละ property ก็จริง แต่ลงที่ค่าคำนวณเดียวกัน และตัวที่ specificity สูงกว่าชนะ
